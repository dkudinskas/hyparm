...
static struct decodingTableEntry t16ArithmeticInstructions[] =
{
  ...
  { FALSE, &t16AddInstruction,       0x1800, 0xFF00, "ADD Rd, Rn, Rm" },
  /*
   * FIXME: impossible
   ...
   */
  // CMP is fine
  { FALSE, &t16CmpInstruction,       0x2800, 0xF800, "CMP<c> Rn, #<imm8>" },
  { FALSE, &t16SubInstruction,       0x1E00, 0xFE00, "SUB{S} <Rd>, <Rn>,#<imm3>" },
  { FALSE, &t16SubInstruction,       0x3800, 0xF800, "SUB{S} <Rdn>, #<imm8>" },
  { FALSE, &t16SubInstruction,       0x1A00, 0xFE00, "SUB{S} <Rd>, <Rn> <Rm>" },
  { FALSE, &t16MovInstruction,       0x2000, 0xF800, "MOV<c> <Rd>, #<imm8>" },
...
static struct decodingTableEntry t16MiscInstructions[] =
{
   ...
   */
  { FALSE, &t16AddInstruction,       0xB000, 0xFF80, "ADD SP, SP, #<imm>" },
...
static struct decodingTableEntry t16SpecialBranchInstructions[] =
{
  ...
  { TRUE,  &t16MovInstruction,       0x4607, 0xFF07, "MOV PC, Rm" },
  { FALSE, &t16MovInstruction,       0x4600, 0xFF00,  "MOV Rd, Rm" },
  /*
   * FIXME: impossible
   * { FALSE, &addInstruction,       0x1800, 0xFE00, "ADD <Rd>, <Rn>, <Rm>" },
   */
  { FALSE, &t16AddInstruction,       0x4400, 0xFF00, "ADD <Rdn>, <Rm>" },
